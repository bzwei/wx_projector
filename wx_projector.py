...
class MainFrame(wx.Frame):
    ...
    def setup_ui(self):
        ...
        self.agenda_btn.SetSize((100, 36))  # Wider to fit text
        ...
        # Version checkboxes and font sizes in the same horizontal layout
        version_font_sizer = wx.BoxSizer(wx.HORIZONTAL)
        ...
        for version_key, version_name in self.bible_versions.items():
            ...
            version_font_sizer.Add(checkbox, 0, wx.RIGHT, 15)
        
        # Add separator
        version_font_sizer.Add(wx.StaticLine(panel, style=wx.LI_VERTICAL), 0, wx.EXPAND | wx.LEFT | wx.RIGHT, 15)
        
        # Add font size controls
        # Chinese font size
        self.chinese_label = wx.StaticText(panel, label="Chinese Font:")
        ...
        self.chinese_font_choice = wx.Choice(panel, choices=['24', '26', '28', '30', '32', '34', '36', '38', '40', '42', '44', '46', '48', '50', '52', '54', '56', '58', '60'], size=(60, -1))
        self.chinese_font_choice.SetSelection(3)  # Default to 30 (index 3)
        self.chinese_font_choice.SetMinSize((60, 30))  # Force minimum width
        ...
        # English font size  
        self.english_label = wx.StaticText(panel, label="English Font:")
        ...
        self.english_font_choice = wx.Choice(panel, choices=['22', '24', '26', '28', '30', '32', '36', '40', '44', '48', '52', '56', '60'], size=(60, -1))
        self.english_font_choice.SetSelection(3)  # Default to 28 (index 3)
        self.english_font_choice.SetMinSize((60, 30))  # Force minimum width
        ...
        version_font_sizer.Add(self.chinese_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        version_font_sizer.Add(self.chinese_font_choice, 0, wx.RIGHT, 20)
        version_font_sizer.Add(self.english_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 5)
        version_font_sizer.Add(self.english_font_choice, 0, 0)
        
        bible_sizer.Add(version_font_sizer, 0, wx.ALL, 10)
    ...
    def update_all_projection_buttons(self):
        """Update all projection buttons based on current active projections"""
        try:
            # Determine what's currently visible on second screen
            url_visible = (self.chrome_process and self.is_projecting) or (self.projection_frame and self.is_projecting and not self.projection_frame.is_hidden)
            slides_visible = self.slides_projection_frame and self.is_projecting_slides and not self.slides_projection_frame.is_hidden
            bible_visible = self.bible_projection_frame and self.is_projecting_bible and not self.bible_projection_frame.is_hidden
            agenda_visible = self.agenda_projection_frame and self.is_projecting_agenda and not self.agenda_projection_frame.is_hidden
            
            # URL/Chrome projection button
            if url_visible:
                self.url_toggle_btn.Enable(True)
                if self.chrome_process:
                    self.url_toggle_btn.SetLabel("🔄 Reload")
                    self.url_toggle_btn.SetBackgroundColour(wx.Colour(52, 152, 219))  # Blue
                else:
                    self.url_toggle_btn.SetLabel("🗕 Hide URL")
                    self.url_toggle_btn.SetBackgroundColour(wx.Colour(243, 156, 18))  # Orange
            else:
                self.url_toggle_btn.Enable(False)
                self.url_toggle_btn.SetLabel("👁 Show URL")
                self.url_toggle_btn.SetBackgroundColour(wx.Colour(95, 95, 95))  # Gray
            
            # Slides projection button
            if slides_visible:
                self.slides_toggle_btn.Enable(True)
                self.slides_toggle_btn.SetLabel("🗕 Hide Slides")
                ...
                if self.slides_projection_frame and self.is_projecting_slides:
                    self.slides_toggle_btn.Enable(True)
                    self.slides_toggle_btn.SetLabel("👁 Show Slides")
                    self.slides_toggle_btn.SetBackgroundColour(wx.Colour(142, 68, 173))  # Purple
                else:
                    self.slides_toggle_btn.Enable(False)
                    self.slides_toggle_btn.SetLabel("👁 Show Slides")
                    self.slides_toggle_btn.SetBackgroundColour(wx.Colour(95, 95, 95))  # Gray
            
            # Bible projection button
            if bible_visible:
                self.bible_toggle_btn.Enable(True)
                self.bible_toggle_btn.SetLabel("🗕 Hide Bible")
                ...
                if self.bible_projection_frame and self.is_projecting_bible:
                    self.bible_toggle_btn.Enable(True)
                    self.bible_toggle_btn.SetLabel("👁 Show Bible")
                    self.bible_toggle_btn.SetBackgroundColour(wx.Colour(39, 174, 96))  # Green
                else:
                    self.bible_toggle_btn.Enable(False)
                    self.bible_toggle_btn.SetLabel("👁 Show Bible")
                    self.bible_toggle_btn.SetBackgroundColour(wx.Colour(95, 95, 95))  # Gray
            
            # Agenda projection button - Always enabled, shows current state
            if agenda_visible:
                self.agenda_btn.SetLabel("🗕 Hide Agenda")
                self.agenda_btn.SetBackgroundColour(wx.Colour(243, 156, 18))  # Orange
            else:
                # If any other projection is visible, or if agenda exists but is hidden, or if no agenda exists
                self.agenda_btn.SetLabel("📋 Show Agenda")
                self.agenda_btn.SetBackgroundColour(wx.Colour(52, 73, 94))  # Dark blue-gray
            
            # Re-enable main action buttons after projections load
            self.main_btn.Enable(True)
            self.slides_btn.Enable(True)
            self.bible_btn.Enable(True)
                    
            # Refresh button displays
            self.url_toggle_btn.Refresh()
            self.slides_toggle_btn.Refresh()
            self.bible_toggle_btn.Refresh()
            self.agenda_btn.Refresh()
            
        except Exception as e:
            print(f"Error updating projection buttons: {e}") 
         
    def refresh_choice_sizing(self):
        """Force proper sizing for choice controls (macOS compatibility)"""
        try:
    ...
    def force_color_updates(self):
        ...
        try:
            ...
            # Restore default font size selections after color updates
...
"""
    ...
    def position_chrome_window(self, target_monitor):
        """Position Chrome window on the target monitor using system-specific methods"""
        try:
            system = platform.system()
            
            if system == "Darwin":  # macOS
                self.position_chrome_window_macos(target_monitor)
            elif system == "Windows":
                self.position_chrome_window_windows(target_monitor)
            else:  # Linux
                self.position_chrome_window_linux(target_monitor)
                
        except Exception as e:
            print(f"Error positioning Chrome window: {e}")
    
    def position_chrome_window_macos(self, target_monitor):
        """Position Chrome window on macOS using AppleScript"""
        try:
            # AppleScript to move Chrome to second monitor and make it fullscreen
            apple_script = f'''
            tell application "Google Chrome"
                activate
                set bounds of front window to {{{target_monitor.x}, {target_monitor.y}, {target_monitor.x + target_monitor.width}, {target_monitor.y + target_monitor.height}}}
                delay 0.5
            ...
            # Try Chromium if Google Chrome doesn't work
            apple_script_chromium = f'''
            ...
            try:
                ...
                print("Chrome positioned using AppleScript (Google Chrome)")
            except:
                # Try Chromium as fallback
                try:
                    subprocess.run(["osascript", "-e", apple_script_chromium], check=True)
                    print("Chrome positioned using AppleScript (Chromium)")
                except:
                    print("AppleScript positioning failed")
                    
        except Exception as e:
            print(f"Error in macOS Chrome positioning: {e}")
    
    def position_chrome_window_windows(self, target_monitor):
        """Position Chrome window on Windows"""
        try:
            import win32gui
            import win32con
            ...
            def enum_windows_callback(hwnd, windows):
                if win32gui.IsWindowVisible(hwnd):
                    ...
                    if "chrome" in window_title.lower() or "Google Chrome" in window_title:
                        windows.append(hwnd)
                return True
            ...
            if windows:
                # Move the most recent Chrome window
                hwnd = windows[-1]
                ...
                print("Chrome positioned using Windows API")
            else:
                print("No Chrome windows found to position")
                
        except ImportError:
            print("pywin32 not available, skipping Windows positioning")
        except Exception as e:
            print(f"Error in Windows Chrome positioning: {e}")
    
    def position_chrome_window_linux(self, target_monitor):
        """Position Chrome window on Linux using wmctrl or xdotool"""
        try:
            # Try wmctrl first
            try:
                subprocess.run([
                    "wmctrl", "-r", "Chrome", "-e", 
                    f"0,{target_monitor.x},{target_monitor.y},{target_monitor.width},{target_monitor.height}"
                ], check=True)
                # Make it fullscreen
                subprocess.run(["wmctrl", "-r", "Chrome", "-b", "add,fullscreen"], check=True)
            ...
            try:
                ...
                if result.stdout:
                    ...
                    subprocess.run([
                        ...
                        str(target_monitor.width), str(target_monitor.height)
                    ])
                    # Make fullscreen
                    ...
                    print("Chrome window not found with xdotool")
            except:
                print("xdotool positioning failed")
                
        except Exception as e:
            print(f"Error in Linux Chrome positioning: {e}")
    ...
    def start_projection(self, url):
        ...
        try:
            ...
            # Stop existing Chrome process
            self.stop_chrome_projection()
                
            # Get target monitor
            monitors = get_monitors()
            target_monitor = monitors[1] if len(monitors) >= 2 else monitors[0]
            
            print(f"Starting Chrome projection to monitor: {target_monitor}")
            
            # Find Chrome executable
            chrome_path = self.find_chrome_executable()
            if not chrome_path:
                # Fallback to embedded WebView
                wx.MessageBox("Chrome browser not found. Using embedded browser instead.", "Chrome Not Found", wx.OK | wx.ICON_WARNING)
                self.start_embedded_projection(url, target_monitor)
                return
            
            # Update status immediately
            self.status_text.SetLabel("Launching Chrome browser on second screen...")
            self.main_btn.Enable(False)
            
            # Prepare Chrome command line arguments (without fullscreen initially)
            chrome_args = [
                chrome_path,
                "--new-window",
            ...
            print(f"Target monitor: {target_monitor.x}x{target_monitor.y} size {target_monitor.width}x{target_monitor.height}")
            
            # Launch Chrome process
            self.chrome_process = subprocess.Popen(chrome_args)
            print(f"Chrome launched with PID: {self.chrome_process.pid}")
            
            # Use system-specific approach to position and fullscreen the window
            wx.CallLater(1000, lambda: self.position_chrome_window(target_monitor))
            
            # Simulate successful loading after positioning
            wx.CallLater(3000, self.on_chrome_projection_loaded)
            
        except Exception as e:
            print(f"Chrome projection error: {e}")
            self.on_projection_error(f"Failed to start Chrome projection: {str(e)}")
    
    def start_embedded_projection(self, url, target_monitor):
        """Fallback to embedded WebView projection"""
        try:
            # Create projection window
            self.projection_frame = ProjectionFrame(self, url, target_monitor, "web")
            
            # Update status
            self.status_text.SetLabel("Loading webpage in embedded browser...")
            
        except Exception as e:
            print(f"Embedded projection error: {e}")
            self.on_projection_error(f"Failed to start embedded projection: {str(e)}")
    
    def on_chrome_projection_loaded(self):
        """Called when Chrome projection is assumed to be loaded"""
        self.is_projecting = True
        self.status_text.SetLabel("✓ Chrome projection active on second screen - Close Chrome window to stop")
        self.status_text.SetForegroundColour(wx.Colour(39, 174, 96))
        ...
        wx.CallLater(1000, self.monitor_chrome_process)
    
    def monitor_chrome_process(self):
        """Monitor Chrome process and update UI when it closes"""
        if self.chrome_process and self.is_projecting:
            poll_result = self.chrome_process.poll()
            if poll_result is not None:
                # Chrome process has ended
                print(f"Chrome process ended with code: {poll_result}")
    ...
    def on_chrome_closed(self):
        """Called when Chrome browser is closed"""
        self.chrome_process = None
        self.is_projecting = False
        
        # Reset UI
        self.status_text.SetLabel("Chrome projection stopped")
        self.status_text.SetForegroundColour(wx.Colour(127, 140, 141))
        
        # Update all projection buttons
        self.update_all_projection_buttons()
    
    def stop_chrome_projection(self):
        """Stop the Chrome projection"""
        if self.chrome_process:
            try:
                self.chrome_process.terminate()
                # Give it a moment to close gracefully
                wx.CallLater(1000, lambda: self.force_close_chrome() if self.chrome_process else None)
            except Exception as e:
                print(f"Error stopping Chrome: {e}")
            
    def force_close_chrome(self):
        """Force close Chrome if it didn't terminate gracefully"""
        if self.chrome_process:
            try:
                self.chrome_process.kill()
                self.chrome_process = None
                print("Chrome process force closed")
            except Exception as e:
                print(f"Error force closing Chrome: {e}")
    ...
    def stop_projection(self):
        ...
        # Stop embedded projection if running
    ...
    def on_url_toggle_click(self, event):
        """Handle URL projection show/hide toggle or reload for Chrome"""
        # If Chrome is running, reload the page
        if self.chrome_process and self.is_projecting:
            current_url = self.url_ctrl.GetValue().strip()
            if current_url:
                # Stop current Chrome and restart with same URL
                self.stop_chrome_projection()
                wx.CallLater(500, lambda: self.start_projection(current_url))
                return
        
        # Handle embedded WebView toggle
    ...
    def hide_agenda_projection(self, auto_hide=False):
        ...
        if self.agenda_projection_frame:
            ...
            if auto_hide:
                self.status_text.SetLabel("📱 Meeting agenda auto-hidden for other projection")
            else:
    ...
    def on_main_window_close(self, event):
        """Handle main window close event - clean up all processes"""
